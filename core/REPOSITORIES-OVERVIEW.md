...
```
Organization: automation-ddt

📁 automation-core-ddt          [Backend - Core Logic]
📁 automation-bff-ddt           [Backend - API Gateway]
📁 automation-worker-android    [Mobile - Worker APK]
...
graph TB
    ...
    WORKER[automation-worker-android<br/>Worker APK]
...
│   │   ├── java/com/automation/
│   │   │   │
│   │   │   ├── AutoUpgradeDdtApplication.java
│   │   │   │
...
│   │   │   │   │   ├── DeviceMapper.java
│   │   │   │   │   ├── OrderMapper.java
│   │   │   │   │   ├── OrderTaskMapper.java
│   │   │   │   │   ├── ServerMapper.java
│   │   │   │   │   └── GroupServerMapper.java
│   │   │   │   │
│   │   │   │   └── services/
│   │   │   │       ├── OrderService.java
...
│   │   │       │           ├── OrderTaskRepositoryImpl.java
│   │   │       │           └── ServerRepositoryImpl.java
│   │   │       │
│   │   │       ├── config/
│   │   │       │   ├── DataLoader.java
│   │   │       │   ├── SwaggerAPI.java
...
│   │   │           │   ├── OrderTaskEntityMapper.java
│   │   │           │   ├── ServerEntityMapper.java
│   │   │           │   └── GroupServerEntityMapper.java
│   │   │           │
│   │   │           └── model/           # Entidades JPA
│   │   │               ├── AccountEntity.java
│   │   │               ├── DeviceEntity.java
│   │   │               ├── OrderEntity.java
│   │   │               ├── OrderTaskEntity.java
│   │   │               ├── ServerEntity.java
│   │   │               └── GroupServerEntity.java
│   │   │
│   │   └── resources/
│   │       ├── application.yml
│   │       ├── application-dev.yml       🆕
...
#### **1. APIs REST para BFF:**
- `GET/POST /api/orders` - Criar e listar ordens
- `GET /api/orders/{id}` - Detalhes de uma ordem
- `GET /api/order-tasks` - Listar tasks de ordens
- `GET /api/accounts` - Gerenciar contas
- `GET /api/devices` - Gerenciar devices
- `GET /api/servers` - Gerenciar servidores

#### **2. APIs REST para Workers:** 🆕
- `GET /api/v1/worker/tasks/available` - Worker pega próxima task
- `POST /api/v1/worker/accounts/request` - Worker solicita conta
- `PUT /api/v1/worker/tasks/{id}/result` - Worker envia resultado
- `POST /api/v1/worker/devices/heartbeat` - Worker reporta status
- `GET /api/v1/worker/config` - Worker obtém configurações
- `POST /api/v1/worker/register` - Registrar novo worker

#### **3. Lógica de Negócio:**
- Desmembrar Orders em OrderTasks
- Alocar Accounts para Tasks
- Gerenciar estado de Tasks (pending, assigned, running, completed, failed)
- Liberar Accounts após conclusão
- Rastreamento de histórico e auditoria
- Validações de regras de negócio

#### **4. Persistência:**
- CRUD completo de todas as entidades
- Queries otimizadas (MyBatis)
- Transações e integridade referencial
- Migrations de banco

#### **5. Fila de Tasks:** 🆕
- Enfileirar tasks pendentes
...
|-------------|-----------|-----------|--------|--------|------------|
| automation-core-ddt | Java | Spring Boot | AWS ECS | ✅ Existe | 🔴 Alta |
| automation-worker-android | Kotlin | Android SDK | APK | 🆕 Criar | 🔴 Alta |
...
1. ✅ automation-core-ddt - Expandir com endpoints para Workers
2. 🆕 automation-worker-android - Criar estrutura básica
3. 🆕 automation-contracts - Definir specs OpenAPI (opcional mas recomendado)
